...
import wandb
from src.utils.click_trainer import trainer, trainer_dr_risk, trainer_dr, trainer_regression
...
wandb.login()
if args.risk == 1:
    wandb.init(project=args.dataset + str(args.noise) + "_risk_T=%0.2f_PBM=1.0_exp"%args.T, entity='e12332263-tu-wien')
else:
    wandb.init(project=args.dataset + str(args.noise) + "_ips_T=%0.2f_PBM=1.0_exp"%args.T, entity='e12332263-tu-wien')
...
def main():
    ...
    if args.risk == 1:
        results_file_risk = open(os.path.join('./results/test_results', args.dataset + '_' + str(args.T) + '_' + str(args.num_sessions) + '_' + str(args.job_id) + '_' + 'dr_risk'), 'w')
