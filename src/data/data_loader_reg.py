...
class ClickLogDataloader(Dataset):
    ...
    def __init__(self, meta_dir=None, click_dir=None, mode='train', feat_vec_dim=None, max_cand_size=None, clip=0.00001, clip_alpha=0.00001, click_model='pbm', estimator='ips'):
        '''
        ...
        if click_model == 'pbm':
            ...
            if self.estimator == 'ips':
                ...
                # self.ips_weights = (self.ctr_map - self.beta_freq)/self.alpha_freq
            else:
        ...
        # flatten the data from query , num_docs -> query * num_docs to train the regression model
        num_query_docs = (len(self.qid_active) * max_cand_size)
        self.qid_feat_tensor = self.qid_feat_tensor.reshape(num_query_docs, -1)
        self.ips_weights = self.ips_weights.reshape(num_query_docs, -1)
        self.reg_weights = self.reg_weights.reshape(num_query_docs, -1)
        self.qid_mask = self.qid_mask.reshape(num_query_docs, -1)
        self.expected_alpha1 = self.expected_alpha1.reshape(num_query_docs, -1)
        self.expected_alpha = self.expected_alpha.reshape(num_query_docs, -1)
        self.expected_beta = self.expected_beta.reshape(num_query_docs, -1)
        self.expected_alpha_beta = self.expected_alpha_beta.reshape(num_query_docs, -1)
        self.display_mask = self.display_mask.reshape(num_query_docs, -1)
        self.alpha_freq = self.alpha_freq.reshape(num_query_docs, -1)
        self.beta_freq = self.beta_freq.reshape(num_query_docs, -1)
    ...
    def __len__(self):
        # total samples = num_queries * num_docs
    ...
    def __getitem__(self, idx):
        ...
        qid_labels = self.ips_weights[idx]
        qid_labels_reg = self.reg_weights[idx]
        qid_mask = self.qid_mask[idx]
        qid_feats = self.qid_feat_tensor[idx, :]
